...
def getCity():
    ...
    for i in f.readlines():
        ...
        cities.append(
            [node_city_val[0], float(node_city_val[1]),
             float(node_city_val[2])]
        )
...
def calcDistance(cities):
    ...
    cityA = cities[0]
    cityB = cities[-1]
    d = math.sqrt(math.pow(cityB[1] - cityA[1], 2) +
                  math.pow(cityB[2] - cityA[2], 2))
    total_sum += d
    return total_sum
    ...
    for i in range(200):
        ...
        for i in range(int((len(population) - 2) / 2)):
            ...
            new_population.append(
                [calcDistance(child_chromosome1), child_chromosome1])
            new_population.append(
                [calcDistance(child_chromosome2), child_chromosome2])
...
def drawMap(city, answer):
    ...
    secend = answer[1][-1]
    plt.plot([first[1], secend[1]], [first[2], secend[2]], "gray")
    plt.xlabel("Unique Bhattarai, 28136/078, Lab No: 21")
    plt.show()
